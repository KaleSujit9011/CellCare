...
file_path = os.path.join(base_dir, "demo/safe_battery.csv")
...
file_path = os.path.join(base_dir, "demo/warning_battery.csv")
...
file_path = os.path.join(base_dir, "demo/dangerous_battery.csv")
dangerous.to_csv(file_path, index=False)

print("Demo CSV files created!")

# Create a CSV with 10 consecutive cycles
battery = pd.read_csv('processed/battery_features.csv')
sequence_sample = battery.iloc[0:10][['C1', 'C2', 'C3', 'C4', 'min_voltage']]
sequence_sample.to_csv('demo/sequence_test.csv', index=False)
print("Created sequence_test.csv with 10 rows")